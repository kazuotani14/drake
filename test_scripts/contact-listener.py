# Starts an LCM subscriber for contact info from Valkyrie or human model simulation


import lcm

# Message info generated by lcm
try:
...
def handle_message(channel, data):
    ...
    msg = lcmt_contact_results_for_viz.decode(data)
    print("    num_contacts  = %s" % str(msg.num_contacts))
    for i, c in enumerate(msg.contact_info):
        print("    Contact %d, between %s and %s, [%f, %f, %f]" % (i, c.body1_name, c.body2_name, c.contact_point[0], c.contact_point[1], c.contact_point[2]))

print("Starting to listen for messages")
lc = lcm.LCM()
subscription = lc.subscribe("CONTACT_RESULTS", handle_message) #,
#subscription = lc.subscribe("EXAMPLE", handle_message) #,
